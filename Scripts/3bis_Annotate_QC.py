...
output_path = os.path.join(script_dir, '..', 'Database', 'annotated_data', 'QC.processed_conf_texts_with_new_responses_2.csv')
...
for index, row in new_df.iterrows():
    ...
    if new_df.at[index, 'detect_COVID_response'] == 'oui':
        for additional_instruction in ['frame', 'measures','associated_emotion']:
            ...
            if additional_instruction == 'frame':
                response = 'dangereux' if re.search('\bdangere', full_response) else 'acceptable' if re.search(r'\bacceptab', full_response) else 'neutre' if re.search(r'\bneutr', full_response) else full_response
                ...
                response = 'négatif' if re.search(r'\bneg', full_response) else 'positif' if re.search(r'\bposi', full_response) else 'neutre' if re.search(r'\bneutr', full_response) else full_response
    ...
    if new_df.at[index, 'detect_evidence_response'] == 'oui' or new_df.at[index, 'detect_source_response'] == 'oui':
        for additional_instruction in ['source_of_evidence', 'associated_emotion', 'country_source', 'frame']:
            prompt = f"Tu es un annotateur de texte en français.\n{df.at[index, additional_instruction]}\n{context}"
            full_response = generate(model, prompt, options=options)['response'].strip().lower()
            full_response = unidecode(full_response)
